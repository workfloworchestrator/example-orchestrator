...
`workflows`, and `db` modules. The `graphql_federation` module provides
customised GraphQL types and models. The application is started with:
...
This example orchestrator demonstrates the local table approach. A `CustomerTable` is added to the database (see `db/models.py`) and populated through a migration with example customers. The default GraphQL resolvers are overridden to read from the database instead of returning the static default.
...
3. **Override the `customer` field on `SubscriptionInterface` and `ProcessType`** using `override_class` (see `graphql_federation.py`). This is the recommended approach from orchestrator-core — it ensures all autogenerated subscription types inherit the override:
    ...
    async def resolve_subscription_customer(root: SubscriptionInterface) -> CustomerType:
        ...
        if customer:
            return CustomerType(customer_id=customer.customer_id, ...)
        return CustomerType(customer_id=root.customer_id, fullname="missing", shortcode="missing")
    ...
    override_class(ProcessType, [customer_process_field])
    ```

4. **Register the custom subscription interface** in `wsgi.py`:
