...
3. **Override the `customer` field on `SubscriptionInterface` and `ProcessType`** using `override_class` (see `graphql_utils/resolvers.py`). This is the recommended approach from orchestrator-core — it ensures all autogenerated subscription types inherit the override:
